	...
	if(window.inputs) {
		for(var i = 0; i < checkboxes.length; i++) {
            if (checkboxes[i].disabled) {
                checkboxes[i]._ca.disabled = true;
                checkboxes[i]._ca.checked = false;
                if(checkboxLabels[i]) {
                    checkboxLabels[i].className += " disabled"
                }
            } else {
                checkboxes[i]._ca.disabled = false;
                if(checkboxLabels[i]) {
		...
		for(var i = 0; i < selects.length; i++) {
            ...
            $('#optionsDiv'+i+' .select-center ul li').remove();
            $('#optionsDiv'+i+' .select-center ul').html(selects[i]._options.innerHTML);
	...
	for (var nf = 0; nf < document.getElementsByTagName("form").length; nf++) {
		if(document.forms[nf].className.indexOf("default") < 0) {
			for(var nfi = 0; nfi < document.forms[nf].getElementsByTagName("input").length; nfi++) {
                if(document.forms[nf].getElementsByTagName("input")[nfi].className.indexOf("default") < 0) {
			...
			for(var nfl = 0; nfl < document.forms[nf].getElementsByTagName("label").length; nfl++) {
                if(document.forms[nf].getElementsByTagName("label")[nfl].className.indexOf("default") < 0) {
			...
			for(var nfs = 0; nfs < document.forms[nf].getElementsByTagName("select").length; nfs++) {
                if(document.forms[nf].getElementsByTagName("select")[nfs].className.indexOf("default") < 0) {
...
function replaceCheckboxes() {
	for (var q = 0; q < checkboxes.length; q++) {
        ...
        if (checkboxes[q].disabled) {
            checkboxArea.disabled = true;
            if(checkboxLabels[q]) {
                checkboxLabels[q].className += " disabled"
            }
...
function checkCheckboxes(who, action) {
    ...
    if (what.disabled != undefined && what.disabled === true){
        return false;
    }
...
function populateSelectOptions(me) {
	...
	for(var w = 0; w < me.options.length; w++) {
        ...
        if (me.options[w].disabled) {
            optionLink.className = 'disable';
...
function selectMe(selectFieldId,linkNo,selectNo,quiet) {
    ...
    if (selectField.options[linkNo].disabled === true) {
        return false;
    }
